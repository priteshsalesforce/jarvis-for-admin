/* =============================================================
   Jarvis · Agentic HR Operations · Onboarding Day (v5)
   -------------------------------------------------------------
   Forked from v4. The shell, sidenav, workspace pages, and home
   strip are the same as v4's IT-manager build; the persona,
   greeting, chiclets, awaiting-approval queue, and "worth
   knowing today" digest have been re-authored for an HR Admin
   (Priya Menon · TechCorp).

   This build adds two new in-chat step types on top of v4's
   engine:
     · approval-card  — itemised approval (rationale + 3 CTAs)
     · task-list      — multi-agent execution grid with
                         progressive row reveal + status pills

   App owns:
     · the persistent left sidenav (collapse, Jarvis toggle)
     · the v4-style steady-state home (greet + chiclets +
       composer + 3-card "today at a glance" strip)
     · the story engine that plays the HR onboarding chapter
       when the home composer detects an onboarding intent
     · the persistent Q&A router (whisper bar) so Priya can
       interrupt to ask a question

   The HR chapter file under `stories/03-hr-priya.js`
   self-registers by pushing to `window.CHAPTERS`. v5 keeps v4's
   workspace pages registered so the sidebar items stay reachable.
   ============================================================= */
...
(() => {
  ...
  // Lightweight markdown for chat bubbles. Escapes HTML, renders
  // **bold** and `code`, and converts blank lines into paragraph
  // breaks (\n\n → <br><br>) and single newlines into soft line
  // breaks. The order matters: HTML escaping must run first so
  // we never leak raw author content into the DOM.
  const md = (s) =>
    String(s)
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
  ...
  // -------------------------------------------------------------
  // Renders an itemised approval with rationale and three CTAs
  // (primary / secondary / tertiary). Each action carries an
  // optional `goto` so the chapter can branch on the user's
  // choice (e.g. "Approve" → execute beats; "Decline" → soft-end;
  // "Ask a question" → Q&A loop). Resolves with the chosen
  // action's id and goto.
  //
  // Step shape:
  //   { type: "approval-card",
  //     eyebrow, title, summary, rationale,
  //     items: [{ label, spec, cost }],
  //     total: "$4,200",
  //     actions: [
  //       { id, label, kind: "primary"|"secondary"|"tertiary", goto },
  //     ],
  //   }
  function renderApprovalCard(step) {
    return new Promise((resolve) => {
      const bubble = jarvisBubble("");
      const card = document.createElement("article");
      card.className = "approval";

      const items = (step.items || []).map((i) => `
        <li class="approval__item">
          <div class="approval__item-label">${md(i.label)}</div>
          ${i.spec ? `<div class="approval__item-spec">${md(i.spec)}</div>` : ""}
          ${i.cost ? `<div class="approval__item-cost">${md(i.cost)}</div>` : ""}
        </li>
      `).join("");

      const actions = (step.actions || []).map((a) => `
        <button type="button"
                class="approval__btn approval__btn--${a.kind || "primary"}"
                data-action-id="${a.id}">
          ${md(a.label)}
        </button>
      `).join("");
      ...
      card.innerHTML = `
        ${step.eyebrow ? `
          <div class="approval__eyebrow">
            <span class="approval__eyebrow-dot" aria-hidden="true"></span>
            ${md(step.eyebrow)}
          </div>
        ` : ""}
        ${step.title ? `<h3 class="approval__title">${md(step.title)}</h3>` : ""}
        ${step.summary ? `<p class="approval__summary">${md(step.summary)}</p>` : ""}
        ${items ? `<ul class="approval__items">${items}</ul>` : ""}
        ${step.total ? `
          <div class="approval__total">
            <span class="approval__total-label">Total</span>
            <span class="approval__total-value">${md(step.total)}</span>
          </div>
        ` : ""}
        ${step.rationale ? `<p class="approval__reason">${md(step.rationale)}</p>` : ""}
        <div class="approval__actions">${actions}</div>
      `;

      bubble.querySelector(".bubble__body").appendChild(card);
      scrollDown();

      card.querySelectorAll(".approval__btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          const id = btn.getAttribute("data-action-id");
          const action = (step.actions || []).find((a) => a.id === id);
          // Disable all CTAs and visually freeze the picked one so
          // the choice is unmistakeable in the saved chat thread.
          card.querySelectorAll(".approval__btn").forEach((b) => {
            b.disabled = true;
          });
  ...
  // Task list (in-chat)
  // -------------------------------------------------------------
  // Multi-agent execution grid — one row per task, columns are
  // Task / Agent / Status. Rows reveal progressively, then their
  // status pill animates from "Queued" to its final state. This
  // is the visual proof that the orchestrator is fanning work out
  // to specialised agents.
  //
  // Step shape:
  //   { type: "task-list",
  //     eyebrow, title,
  //     rows: [{ task, sub, agent, status: "done"|"pending"|"ordered", final }],
  //     stagger: 220, settle: 360, footnote }
  async function renderTaskList(step) {
    const bubble = jarvisBubble("");
    const wrap = document.createElement("section");
    wrap.className = "task-list";

    const rows = (step.rows || []).map((r, i) => `
      <div class="task-list__row task-list__rows" data-i="${i}" data-status="${r.status || "done"}">
        <div class="task-list__cell task-list__task">
          <div class="task-list__task-label">${md(r.task)}</div>
          ${r.sub ? `<div class="task-list__note">${md(r.sub)}</div>` : ""}
        </div>
        <div class="task-list__cell task-list__agent">${md(r.agent)}</div>
        <div class="task-list__cell">
          <span class="task-list__pill task-list__pill--queued">Queued</span>
          <span class="task-list__pill task-list__pill--final task-list__pill--${r.status || "done"}">
            ${md(r.final || (r.status === "pending" ? "Pending" : r.status === "ordered" ? "Ordered" : "Done"))}
          </span>
        </div>
      </div>
    `).join("");
    ...
    wrap.innerHTML = `
      ${step.eyebrow ? `
        <div class="task-list__eyebrow">
          <span class="task-list__eyebrow-dot" aria-hidden="true"></span>
          ${md(step.eyebrow)}
        </div>
      ` : ""}
      ${step.title ? `<h3 class="task-list__title">${md(step.title)}</h3>` : ""}
      <div class="task-list__head">
        <div class="task-list__row task-list__row--head">
          <div class="task-list__col">Task</div>
          <div class="task-list__col">Agent</div>
          <div class="task-list__col">Status</div>
        </div>
        <div class="task-list__rows-wrap">${rows}</div>
      </div>
      ${step.footnote ? `<p class="task-list__foot">${md(step.footnote)}</p>` : ""}
    `;

    bubble.querySelector(".bubble__body").appendChild(wrap);
    scrollDown();

    const stagger = typeof step.stagger === "number" ? step.stagger : 220;
    const settle  = typeof step.settle  === "number" ? step.settle  : 360;
    const $rows   = Array.from(wrap.querySelectorAll(".task-list__rows-wrap > .task-list__row"));
    ...
    for (const row of $rows) {
      ...
      await sleep(Math.max(120, settle - stagger));
    }
  }
  ...
  async function runStep(story, step) {
    ...
    switch (step.type) {
      ...
      case "approval-card": {
        const picked = await renderApprovalCard(step);
        if (picked.goto) {
          const idx = findStepIndex(story, picked.goto);
          if (idx >= 0) { storyIndex = idx; return true; }
        }
      ...
      case "task-list":     await renderTaskList(step); break;
  ...
  async function runStory(story) {
    ...
    while (storyIndex < story.length) {
      ...
      if (advanced === true && (step.type === "ask" || step.type === "choose" || step.type === "multi-choose" || step.type === "approval-card" || step.type === "goto")) {
  ...
  // End-of-Chapter (v5)
  // -------------------------------------------------------------
  // v5's HR onboarding chapter ends in chat, with no sidebar
  // unlocks (the workspace items were already revealed on home).
  // Drop a quiet director note that names what just landed and
  // offers a one-click jump back to home.
  function renderEndOfChapter() {
    pushDirectorNote({
      title: "Onboarding complete",
      text: "Rohan's Day-1 setup is queued. 8 of 9 agents done; MDM auto-enrols on first sign-in.",
      sub: "Open the activity log to drill in or roll anything back.",
      autoDismiss: 6800,
      actions: [
        { label: "Back to home", primary: true,
          onClick: () => startHome() },
      ],
    });
  }
  ...
  const WORKSPACE_PAGES = {
    ...
    insight: {
      title: "Insight",
      lead: "Headcount, attrition and engagement signals across the people graph.",
      hint: "Design coming soon.",
    },
    horizon: {
      title: "Horizon",
      lead: "Long-range planning. Pipeline, hiring plans and ramp forecasts.",
      hint: "Design coming soon.",
    },
    // ---- Chat-shortcut destinations --------------------------------
    // v5 doesn't open these as right-panel pages (the chiclets stay
    // in chat), but the keys are kept registered so any deep link
    // or future extension lands on a coherent placeholder rather
    // than an empty panel.
    approvals: {
      title: "Today's approvals",
      lead: "HR sign-offs waiting on you \u2014 hardware, contracts, FNF, access grants.",
      hint: "The strip on home shows the top 2; this view is the full queue.",
    },
    joiners: {
      title: "Joiners this week",
      lead: "Cohort view across the upcoming start dates.",
      hint: "Design coming soon \u2014 the home digest carries the headlines.",
    },
    policy: {
      title: "Policy library",
      lead: "Living policy docs \u2014 remote work, leave, perks, code of conduct.",
      hint: "Design coming soon.",
    },
    lens: {
      title: "Lens",
      lead: "Every work item your digital agents are handling, end-to-end.",
      hint: "Filter by Incidents, Cases, Workflows or Changes \u2014 click a row to drill in.",
    },
    cmdb: {
      title: "CMDB",
    ...
    studio: {
      title: "Studio",
      lead: "Build, tune, and govern the agents that run your operations.",
      hint: "Design coming soon.",
    },
    reports: {
      title: "Reports",
      lead: "Metrics, rollups, and exports across your IT graph.",
      hint: "Design coming soon.",
    },
    "workitem-detail": {
      title: "Work item",
      lead: "Live agent orchestration timeline for this work item.",
      hint: "Open from any Lens row to follow the agent handoffs in real time.",
    },
    // Legacy incident surface kept registered so any v4 share link
    // pointing here still mounts the placeholder rather than 404'ing.
    "incident-live": {
      title: "Live incident",
      lead: "Live operational incident timeline.",
      hint: "Not used in v5's HR flow.",
    },
  };
  ...
  // v6 — John Mathew runs a fleet of digital agents at IndiaFirst Bank.
  const HOME_PERSONA = {
    name: "John",
    role: "IT Manager",
    org: "IndiaFirst Bank",
  ...
  // Quick-action shortcuts. v5 differs from v4: instead of opening
  // a right-panel page, every chiclet drops a canonical prompt into
  // the chat. The "onboard" chiclet matches the HR chapter intent
  // and replays the full onboarding story; the rest fall through
  // to the FAQ router, which lets Priya feel the demo's depth
  ...
  // Awaiting-approval rows — HR work that's queued for Priya's
  // sign-off. Same .signoff vocabulary as v4 so the bottom-strip
  // card renders with the same density / hierarchy. The "high"
  // risk band is reserved for items where a delay materially
  // affects the candidate's day-one (e.g. hardware that ships
  // from Singapore takes a week, or a contract that expires).
  const HOME_SIGNOFFS = [
    {
  ...
  // People-side signals trending the wrong way. Each row reads as a
  // plain-English headline ("Mid-month attrition risk: 3 engineers
  // open to leave") and a sub line with the numeric framing. This
  // is the HR equivalent of v4's "services to watch" — what should
  // be on Priya's radar even though no decision is pending.
  const HOME_TRENDING = [
    {
  ...
  // moments, completed sweeps. The HR equivalent of v4's "Worth
  // knowing today": signals Priya likes to know but doesn't have
  // to act on right now.
  const HOME_DIGEST = [
    {
      tone: "schedule",
  ...
  // Per-chiclet context line shown as the first Jarvis bubble when
  // a chiclet's canned prompt enters chat. Kept short — the next
  // beat (the chapter, or the FAQ fallback) carries the substance.
  const CHICLET_CONTEXT = {
    onboard:    `Kicking off **Rohan's onboarding** \u2014 fetching the position listing, reading the role's hardware needs, and routing the approval that needs your eye.`,
    approvals:  `**4 items** are waiting for your decision. Top of the queue: Rohan's specialist hardware. Approving releases the agent's plan; deferring buys you 24 hours.`,
    joiners:    `**3 joiners** start Monday. The pre-boarding portal, welcome kits and day-1 invites are all lined up. I'll flag the one going amber.`,
    policy:     `Looking up the **remote-work policy** for new hires \u2014 hybrid baseline, India-specific clauses, and any role-level exceptions for Platform Eng.`,
  };
  ...
  const renderHomeTrend = (t) => {
    ...
    const interactive = t.tone === "high";
    // Interactive rows behave as buttons (keyboard reachable +
    // assistive-tech announceable) while staying inside the
    // semantic <ul>. The click delegate on .home reads
    // [data-home-trend] to decide where to route.
    const a11y = interactive
      ? `role="button" tabindex="0" data-home-trend="incident" aria-label="Open incident: ${esc(t.title)}"`
      : "";
    const cls = "home-trends__row" + (interactive ? " home-trends__row--clickable" : "");
    return `
      <li class="${cls}" data-tone="${esc(t.tone)}" ${a11y}>
  ...
  function renderHome() {
    ...
    const titleTail = "Agentic HR Operations";
    const placeholder = "Ask Jarvis to onboard, offboard, approve, or look something up\u2026";
    ...
    home.innerHTML = `
      <div class="home__inner">
        ...
        <div class="home__strip" role="region" aria-label="Today at a glance">
          ...
          <!-- On your radar — HR equivalent of v4's "Services to watch".
               Surfaces people / vendor signals trending the wrong way
               so Priya knows them before they become approvals. -->
          <section class="home-card home-card--trending">
            <header class="home-card__head">
              <span class="home-card__eyebrow">On your radar</span>
            </header>
            <ul class="home-card__body home-trends">
              ${HOME_TRENDING.slice(0, 2).map(renderHomeTrend).join("")}
            </ul>
            ${renderHomeMore("joiners", "See this week's joiners")}
          </section>
          ...
          <section class="home-card home-card--digest">
            ...
            ${renderHomeMore("policy", "Open the full digest")}
          </section>
        </div>
      </div>
    `;
    ...
    // chat mode. If the typed text matches the onboarding intent
    // (keywords: onboard / new hire / position-listing id / Rohan)
    // we replay the HR chapter; otherwise we hand off to the FAQ
    // router so Priya still gets a coherent answer.
    const submit = () => {
      const $input = home.querySelector("#homeWhisperInput");
      const text = ($input?.value || "").trim();
      if (!text) return;
      $input.value = "";
      const matchesOnboardingIntent =
        /\b(onboard|new hire|joiner|pl-?\s*\d{4,}|rohan)\b/i.test(text);
      home.classList.add("home--leaving");
      setTimeout(() => {
        if (home.parentNode) home.remove();
        enterChatFromHome(text, {
          intent: matchesOnboardingIntent ? "chapter" : "fallback",
        });
      }, 280);
    };
  ...
  // Chiclet → drop a canonical prompt into chat. v5 is a chat-first
  // demo: instead of opening a right-panel page (v4's behaviour),
  // every chiclet types its preset prompt as the user, then either
  // launches the HR onboarding chapter (when the chiclet matches
  // the onboarding intent) or falls back to the FAQ router.
  function openFromHome(chicletId) {
    ...
    if (home) {
      ...
      setTimeout(() => {
        if (home.parentNode) home.remove();
        enterChatFromHome(chiclet.prompt || chiclet.label, {
          chicletId,
          intent: chicletId === "onboard" ? "chapter" : "fallback",
        });
      }, 280);
    } else {
      // No home in the DOM (defensive — shouldn't normally happen).
      enterChatFromHome(chiclet.prompt || chiclet.label, {
        chicletId,
        intent: chicletId === "onboard" ? "chapter" : "fallback",
      });
  ...
  // Shared transition: home → chat. Renders the user bubble for
  // whatever Priya typed (or the chiclet's canned prompt), then
  // hands off to the chapter (onboarding intent) or the FAQ
  // router (everything else). The chiclet path optionally posts a
  // CHICLET_CONTEXT line as the opening Jarvis bubble so the chat
  // doesn't feel cold.
  function enterChatFromHome(text, opts = {}) {
    const { chicletId = null, intent = "fallback" } = opts;
    ...
    // Onboarding intent — replay the HR chapter. The chapter's
    // story array starts post-greet (no welcome / no user bubble),
    // so the user bubble we just rendered survives and the chapter
    // beats follow naturally.
    if (intent === "chapter") {
      runHrChapter();
      return;
    }

    // Everything else — the canned context line (when we know the
    // chiclet) followed by the FAQ router's fallback. This keeps
    // every chiclet feeling responsive without claiming depth we
    // don't have wired up yet.
    if (chicletId && CHICLET_CONTEXT[chicletId]) {
      streamJarvis(md(CHICLET_CONTEXT[chicletId]));
      lastPersona = "jarvis";
    } else {
      renderFallback();
    }
  ...
  // Launch the HR onboarding chapter from the current chat thread
  // (without wiping it — the user bubble that triggered us must
  // survive). Mirrors what startSetup() does, minus the thread
  // reset.
  function runHrChapter() {
    const chapter = findChapter("hr-priya");
    if (!chapter) {
      console.warn("HR chapter not found");
      renderFallback();
      return;
    }
    runToken += 1;
  ...
  function startHome() {
    ...
    // Home is the active rail item while the chat surface is visible.
  ...
  // Re-enter the HR onboarding chapter from a clean thread
  // (used by Settings → Re-run setup and the #setup escape hatch).
  // The home composer / chiclets use runHrChapter() instead, which
  // preserves the user bubble that triggered the chapter.
  function startSetup() {
    const chapter = findChapter("hr-priya");
    if (!chapter) {
      console.warn("HR chapter not found");
  ...
  async function renderFallback() {
    ...
    await sleep(450);
    const html = md(
      "I'm tracking every work item your digital agents are handling. " +
      "Open the **Lens** for the live feed \u2014 incidents, cases, workflows " +
      "and changes \u2014 or ask me about a specific work item."
    );
    await streamInto(typing, html);
  }
  ...
  async function handleWhisper(rawText) {
    ...
    // Lens shortcuts — typing anything that mentions "lens" or
    // a work-item routing keyword takes John straight to the Lens
    // (full canvas) without opening a chat thread he doesn't need.
    if (/\b(lens|work ?item|workflow|incident|loan|sanction|rohan)\b/i.test(text)) {
      setChatClosed(true);
      navigateTo("lens");
      return;
    }
  ...
  // chat thread and lands the user back on the Lens (v6's home
  // surface). Tears down any running incident first so the
  ...
  $stageNewChat?.addEventListener("click", () => {
    ...
    pushDirectorNote({
      title: "New chat",
      text:  "Started a fresh session. You're back on the Lens.",
      autoDismiss: 2000,
    });
  });
  ...
  // `private: true` adds a small lock glyph next to the title —
  // mirrors how Slack flags restricted channels. Curated to fit
  // Priya's HR-admin persona (TechCorp · Platform Eng & beyond).
  const CHAT_HISTORY = [
    {
      date: "Tuesday, May 12th",
      items: [
        {
          title: "Onboard Rohan Sharma — PL-13234, Software Architect",
          preview: "Got it. Pulled the position listing, hardware spec is non-standard — routing approval to you.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Offboarding checklist for Karthik R. — last day May 24",
          preview: "Final-pay calc, asset return, exit interview, GitHub revoke — staged. Awaiting your sign-off on FNF.",
          private: true,
        },
        {
          title: "Q2 performance-review templates — publish to all managers",
          preview: "Templates pushed to Workday. 86 managers in scope. Slack nudges scheduled for Mon 10:30.",
        },
        {
          title: "Maya needs Workday admin — what's the right approval path?",
          preview: "It's an HRBP-level grant, so it routes to you (admin owner) and Saanvi for the cost-centre tag.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Summarise the engagement-survey results for Platform Eng",
          preview: "eNPS up 6 pts QoQ. Top driver: career-growth conversations. Lowest: workload predictability.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Who's joining next week — full picture",
          preview: "3 joiners on May 25: Rohan (Platform Eng), Aditi (Design), Samir (FinOps). All on track.",
        },
        {
          title: "What's our remote-work policy for new hires?",
          preview: "Hybrid baseline: 3 days in office in the first 90 days. India-specific clauses on stipend + ergonomics.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Stay-interview prep — 3 amber engineers in Platform Eng",
          preview: "Compiled their 90-day signals: 1:1 cadence, last promo, recent project wins. Talking points ready.",
        },
        {
          title: "Look at attrition trend — last 6 months by team",
          preview: "Overall down 0.8 pts MoM. Platform Eng -2.1; Sales -0.4; FinOps flat. Detail in Lens.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Auto-rotate the background-check vendor — Verifast SLA slipping",
          preview: "Drafting RFP for two backups (HireRight + Sterling). Comparison ready by Wednesday.",
          private: true,
        },
        {
          title: "Promotion cycle — what's our cadence?",
          preview: "Twice a year (Q2 + Q4). Calibration with leadership 4 weeks before, then comm letters.",
        },
      ],
    },
  ];
  ...
  // returns to the steady-state home view.
  ...
  $navProfile?.addEventListener("click", () => {
    ...
    pushDirectorNote({
      title: "Profile",
      text: "John Mathew · IT Manager · IndiaFirst Bank.",
  ...
  (function bootstrap() {
    ...
    if (deepLinkPage) {
      ...
      // Escape hatch — replay the chapter from a clean thread.
      setChatClosed(false, { instant: true });
      applySidenavGating();
      startSetup();
    } else {
      // v6 default — boot John straight into the Lens view (full
      // canvas, chat collapsed). Lens is John's primary surface
      // because his job IS to keep an eye on what the digital
      // agents are doing across the bank's IT graph.
      setupComplete = true;
      applySidenavGating();
      setChatClosed(true, { instant: true });
      $workspace.classList.remove("workspace--welcome");
