...
(() => {
  ...
  async function renderFallback() {
    ...
    await sleep(450);
    const html = md(
      "I'm here to help with onboarding, offboarding, hardware approvals, " +
      "access requests, and HR policy lookups. Try \u201cOnboard a new hire\u201d " +
      "or ask about today's approvals."
    );
    await streamInto(typing, html);
  }
  ...
  async function handleWhisper(rawText) {
    ...
    // Dock-side onboarding-intent capture — same regex as the home
    // composer. Lets Priya kick off the chapter mid-session
    // (e.g. after a fallback answer) without going back to home.
    if (/\b(onboard|new hire|joiner|pl-?\s*\d{4,}|rohan)\b/i.test(text)) {
      runHrChapter();
      return;
    }

    const faqs = (findChapter("hr-priya")?.faqs) || [];
  ...
  $navProfile?.addEventListener("click", () => {
    ...
    pushDirectorNote({
      title: "Profile",
      text: "Priya Menon · HR Admin · TechCorp.",
  ...
  (function bootstrap() {
    ...
    if (deepLinkPage) {
      ...
      // v5 default — drop Priya onto the steady-state home
      // (greet + chiclets + composer + 3-card "today at a
      // glance" strip).
      startHome();
    }
  })();
