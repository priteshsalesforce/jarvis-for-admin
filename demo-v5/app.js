/* =============================================================
   Jarvis · v5 Story Engine + Sidenav Router (HR Admin · Priya)
   -------------------------------------------------------------
   This build is scoped to the HR-admin Onboarding Day chapter
   (Priya Menon at TechCorp). The fullscreen-chat layout from v3
   is preserved — workspace surfaces are intentionally not used,
   so the entire flow lives in the Jarvis chat thread.

   This file owns:
     · the persistent left sidenav (collapse + Jarvis toggle)
     · auto-launch of the Onboarding chapter on first paint
     · the story engine that plays the chapter beat-by-beat,
       including two v5-only step types:
         - approval-card  · rich approval surface (rationale +
                            itemised cost + 3 CTAs)
         - task-list      · multi-row outcome grid with
                            progressive row reveal
     · the persistent Q&A router (whisper bar) so Priya can
       interrupt the story with a free-text question

   The chapter file under `stories/03-hr-priya.js` self-registers
   by pushing to `window.CHAPTERS`. Workspace pages aren't used
   in v5, so `window.PAGES` is unused.
   ============================================================= */
...
(() => {
  ...
  // Tiny markdown-ish helper. Escapes the unsafe HTML characters
  // first, then converts the small subset of inline markdown the
  // chapter authors actually rely on:
  //   • **bold**            → <strong>bold</strong>
  //   • `code`              → <code>code</code>
  //   • blank line (\n\n)   → paragraph break (visual gap)
  //   • single newline (\n) → soft line break (one <br>)
  // Line-break support keeps multi-line say steps in v5 readable
  // without forcing chapter authors to split every paragraph into
  // its own step.
  const md = (s) =>
    String(s)
      .replace(/&/g, "&amp;")
  ...
  // -------------------------------------------------------------
  // approval-card — rich approval surface used by the v5 HR
  // chapter. Renders an itemised cost breakdown + rationale and
  // exposes Approve / Decline / Ask-a-question CTAs (the third
  // CTA is opted-in via `tertiary: true` so existing two-button
  // shapes still work). Returns the picked choice so the engine
  // can honour `goto`.
  // -------------------------------------------------------------
    ...
    return new Promise((resolve) => {
      ...
      const itemsHtml = (step.items || []).map((it) => `
        <li class="approval__item">
          ...
          ${it.spec ? `<span class="approval__item-spec">${md(it.spec)}</span>` : ""}
          ${it.cost ? `<span class="approval__item-cost">${md(it.cost)}</span>` : ""}
        </li>
      `).join("");

      const totalHtml = step.total
      ...
      const reasonHtml = step.reason
        ? `<p class="approval__reason">${md(step.reason)}</p>`
        : "";
      ...
      card.innerHTML = `
        ${eyebrowHtml}
        <h3 class="approval__title">${md(step.title || "Approval requested")}</h3>
        ${step.summary ? `<p class="approval__summary">${md(step.summary)}</p>` : ""}
        ${itemsHtml ? `<ul class="approval__items">${itemsHtml}</ul>` : ""}
        ${totalHtml}
        ${reasonHtml}
        <div class="approval__actions"></div>
      `;

      const actionsEl = card.querySelector(".approval__actions");
      (step.choices || []).forEach((c) => {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className =
          "approval__btn" +
          (c.primary ? " approval__btn--primary" : "") +
          (c.tertiary ? " approval__btn--tertiary" : "") +
          (!c.primary && !c.tertiary ? " approval__btn--secondary" : "") +
          (c.danger ? " approval__btn--danger" : "");
        btn.innerHTML = `
          ${c.glyph ? `<span class="approval__btn-glyph" aria-hidden="true">${c.glyph}</span>` : ""}
          <span class="approval__btn-label">${md(c.label)}</span>
        `;
        btn.addEventListener("click", () => {
          [...actionsEl.children].forEach((b) => (b.disabled = true));
          btn.classList.add("is-selected");
          userBubble(c.userText || c.label);
          resolve(c);
        });
  ...
  // -------------------------------------------------------------
  // task-list — multi-row outcome grid used after the approval
  // lands. Reveals rows progressively (each row's status pill
  // animates from "queued" → its final state) so the agent
  // fan-out feels live rather than being one big static dump.
  // -------------------------------------------------------------
  async function renderTaskList(step) {
    const bubble = jarvisBubble("");
    const board = document.createElement("article");
    board.className = "task-list";
    ...
    const headHtml = `
      <header class="task-list__head">
        <span class="task-list__col task-list__col--task">Task</span>
        <span class="task-list__col task-list__col--agent">Agent</span>
        <span class="task-list__col task-list__col--status">Status</span>
      </header>
    `;

    // Each row starts in the "queued" pre-reveal state and we mark
    // them `is-revealed` one by one so the page reads as live work.
    const rows = step.rows || [];
    const rowsHtml = rows.map((r, i) => {
      const status = r.status || "done";
      const noteHtml = r.note
        ? `<span class="task-list__note">${md(r.note)}</span>`
        : "";
      const statusLabel =
        status === "done"     ? "Done"
      ...
      return `
        <div class="task-list__row" data-row="${i}">
          <span class="task-list__cell task-list__cell--task">
            <span class="task-list__task">${md(r.task)}</span>
            ${noteHtml}
          </span>
          <span class="task-list__cell task-list__cell--agent">${md(r.agent)}</span>
          <span class="task-list__cell task-list__cell--status">
            <span class="task-list__pill task-list__pill--queued" data-final="${status}">
              <span class="task-list__pill-dot" aria-hidden="true"></span>
              <span class="task-list__pill-label">Queued</span>
              <span class="task-list__pill-final" data-status="${status}">${statusLabel}</span>
            </span>
          </span>
        </div>
      `;
    ...
    board.innerHTML = `
      ${eyebrowHtml}
      ${headHtml}
      <div class="task-list__rows">${rowsHtml}</div>
      ${step.footnote ? `<div class="task-list__foot">${md(step.footnote)}</div>` : ""}
    `;

    bubble.querySelector(".bubble__body").appendChild(board);
    scrollDown();

    // Progressive reveal — each row gets `is-revealed` after a
    // short stagger; once revealed, the queued pill fades out and
    // the final-status pill takes over with a soft pop.
    const revealMs = step.revealMs || 140;
    const rowEls = board.querySelectorAll(".task-list__row");
    for (let i = 0; i < rowEls.length; i++) {
      await sleep(revealMs);
      rowEls[i].classList.add("is-revealed");
      // Re-pin the chat scroll so each new row stays in view as
      // the list grows — important on smaller viewports.
      scrollDown();
    }

    // Hold so the next `say` step doesn't crowd the grid.
  ...
  async function runStory(story) {
    ...
    while (storyIndex < story.length) {
      ...
      if (advanced === true && (
            step.type === "ask"          ||
            step.type === "choose"       ||
            step.type === "multi-choose" ||
            step.type === "goto"         ||
            step.type === "approval-card"
          )) {
  ...
  // -------------------------------------------------------------
  // End-of-chapter — for v5 the chapter is the HR onboarding flow,
  // so there are no workspace surfaces to unlock. We post a quiet
  // director note that names what just landed (Rohan's onboarding
  // is in flight) and leave the sidenav alone so the focus stays
  // on the chat thread.
  // -------------------------------------------------------------
  function renderEndOfChapter() {
    ...
    pushDirectorNote({
      title: chapter.endTitle || "Onboarding kicked off",
      text:
        chapter.endline ||
        "Nine specialist agents are executing in parallel. " +
        "I'll ping you on the pending items.",
      sub: chapter.endSub || "All actions reversible from the audit log.",
    });
  }
  ...
  const WORKSPACE_PAGES = {
    tower: {
      ...
      hint: "Design coming soon — this is the post-onboarding landing surface for Priya.",
    },
    insight: {
      title: "Insight",
  ...
  // Re-enter the chapter. Resets the story and re-runs from the
  // beginning. v5 boots Priya's HR onboarding chapter; v3's IT-admin
  // build used "admin-sarah" instead.
  function startSetup() {
    const chapter = findChapter("hr-priya");
    if (!chapter) {
      console.warn("Onboarding chapter not found");
      return;
    }
    runToken += 1;
    mode = "setup";
  ...
  async function renderFallback() {
    ...
    await sleep(450);
    const html = md(
      "I'm here to help across HR — onboarding, offboarding, hardware approvals, " +
      "access requests, and anything in between. Ask me about a joiner, an open " +
      "approval, or what each agent in the Onboarding Orchestrator does."
    );
    typing.querySelector(".bubble__text").innerHTML = html;
  }
  ...
  // conversations that fit Priya's HR-admin persona (joiners,
  // approvals, off-boarding, policy lookups, performance reviews).
  const CHAT_HISTORY = [
    {
      date: "Tuesday, May 12th",
      items: [
        {
          title: "Reschedule Anika's first-day welcome to Wed",
          preview: "Done — pushed her welcome sync and updated her buddy's calendar invite.",
        },
        {
          title: "Pull the H1 attrition summary",
          preview: "9.8% voluntary, down 1.4 pts QoQ. Engineering is the biggest mover.",
          private: true,
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Offboard Maya — last day next Friday",
          preview: "Knowledge-handoff doc drafted, access revocation queued for May 16 EOD.",
          private: true,
        },
        {
          title: "What's the policy on remote-first hires in EU?",
          preview: "Remote-first is allowed in 14 of 17 EU countries — the gating set is payroll.",
        },
        {
          title: "Approve laptop swap for Karthik",
          preview: "Standard L3 spec, in-budget — approved and shipped to his Bengaluru address.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Draft Q2 performance review templates",
          preview: "Three drafts ready: Engineering, GTM, and Customer Success. Want to preview?",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Background check stuck for Rohan Sharma",
          preview: "Vendor flagged a name-mismatch — re-submitted with the legal-name correction.",
          private: true,
        },
        {
          title: "How long does the L4 hardware approval take?",
          preview: "Median 4h 12m end-to-end, with 80% closing the same business day.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Onboarding launch — Platform Eng cohort",
          preview: "8 joiners across May 18–25. I've staged welcome packs and Day-1 schedules.",
        },
        {
          title: "Open headcount across HR ops",
          preview: "Three open reqs: HRBP (US-WEST), People Ops Manager, and L&D Lead.",
        },
      ],
    },
    {
      ...
      items: [
        {
          title: "Auto-revoke offboarded folks > 30 days",
          preview: "Found 4 lingering accounts. Drafted revocation tickets for your review.",
          private: true,
        },
        {
          title: "Average time-to-onboard last quarter?",
          preview: "Median 3.2 days from offer-accept to Day-1 ready — a 41% drop YoY.",
        },
      ],
    },
  ];
  ...
  // -------------------------------------------------------------
  // Bootstrap — restore sidenav state and auto-launch the
  // Onboarding chapter on first paint. v5 has no workspace
  // surfaces so there's no deep-link router and the gated items
  // stay hidden throughout the experience.
  // -------------------------------------------------------------
  (function bootstrap() {
